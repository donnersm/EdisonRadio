...
#define dsp_white()             tft->fillScreen ( WHITE ) ;         //  clear screen white background
...
#define dsp_drawimg()           tft->drawJpgFile();
...
void displayvolume()
{
  if ( tft )
  {
    ...
    if ( newvol != oldvol )                             // Volume changed?
    {
      ...
      dsp_fillRect ( 0, dsp_getheight() - 5,
                     len, 5, RED ) ;                    // Paint red part
      dsp_fillRect ( len, dsp_getheight() - 5,
                     dsp_getwidth() - len, 5, GREEN ) ; // Paint green part
    }
  }
}
...
void displaytime ( const char* str, uint16_t color )
{
  ...
  if ( tft )                                       // TFT active?
  {
    ...
    for ( i = 0 ; i < 8 ; i++ )                    // Compare old and new
    {
      ...
      {
        dsp_fillRect ( pos, 0, 11, 14, BLACK ) ;     // Clear the space for new character  0,6,8
