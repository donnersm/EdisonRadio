...
//*                 By Ed Smallenburg.                                                              *
//***************************************************************************************************
//*    Modified according to GNU GENERAL PUBLIC LICENSE to my needs                                 *
//*    Mark Donners AKA  The Electronic Engineer                                                    *
//*    www.thelectronicengineer.nl                                                                  *
//***************************************************************************************************
//
...
//***************************************************************************************************
// 16-12-2020 Code takeover modified last version from ES and combined it with code form LOTFI:   
//            Spectrum Analyzer, changed width volume bar, implemented a search tab to find stations
//
// Define the version number, also used for webserver as Last-Modified header and to
// check version for update.  The format must be exactly as specified by the HTTP standard!
#define VERSION     "Wed, 03 Feb 2021 21:16:00 GMT"
// ESP32-Radio can be updated (OTA) to the latest version from a remote server.
// The download uses the following server and files:
#define UPDATEHOST  "judoles.nl"                    // Host for software updates
...
String            hostmem;                               // used to keep the orignal url in mem, in case of redirect!
...
int      n, height;
//*******************************************************************************************
//*******************variables and definitions for VU meter and Logo backlight **************
//*******************************************************************************************
#define N_PIXELS  32  // Number of pixels in strand VUmeter
#define LED_PIN    33  // NeoPixel LED strand is connected to this pin
//#define MIC_PIN   32  // Microphone is attached to this analog pin
#define LED_PIN_LOGO    32  // NeoPixel LED strand is connected to this pin 
#define N_PIXELS_LOGO  12  // Number of pixels in strand
//#define MIC_PIN   32  // Microphone is attached to this analog pin


#define DC_OFFSET  0  // DC offset in mic signal - if unusure, leave 0
#define NOISE     25  // Noise/hum/interference in mic signal 167
#define SAMPLES   50  // Length of buffer for dynamic level adjustment 50
#define TOP       (N_PIXELS +1 ) // Allow dot to go slightly off scale 66
#define PEAK_FALL 6 // Rate of peak falling dot 30
#define PIXBRIGHTNESS 64
#define PIXBRIGHTNESSLOGO 64
#define Brightpin 34

Adafruit_NeoPixel
  strip = Adafruit_NeoPixel(N_PIXELS, LED_PIN, NEO_GRB + NEO_KHZ800);
Adafruit_NeoPixel  
  striplogo = Adafruit_NeoPixel(N_PIXELS_LOGO, LED_PIN_LOGO, NEO_GRB + NEO_KHZ800);
byte
  peak      = 0,      // Used for falling dot 0
  dotCount  = 0,      // Frame counter for delaying dot-falling speed 0
  volCount  = 0;      // Frame counter for storing past volume data 0
int
  vol[SAMPLES],       // Collection of prior volume samples
  lvl       = 5,      // Current "dampened" audio level 5
...
//int lvl = 0, minLvl = 0, maxLvl = 35; // tweak the min and max as needed
...
class VS1053
{
  ...
  protected:
    ...
    { short pseu=1;
      ...
      {
    //    NOP() ;                                   // Very short delay doesnt work in combi with fastlib library
    
    pseu=0;                                       //instead replace by pseudo commmand: resetting a short integer.
      }
    }
...
void VS1053::begin()
{
 ...
 uint16_t status_waarde;
 #define SS_VU_ENABLE 0x0200 // B0000001000000000 (7-de bit van links is 1);  
...
{ int VU_tot=0;
    ...
    for (uint8_t i = 0; i < 14; i++) {
        ...
        spectrum[i][0] = cur; 
        //if (cur>Vu_waarde)Vu_waarde=cur; //werkt maar representatie is niet voldoende
        VU_tot=VU_tot+cur; //werkt maar representatie is niet voldoende
    }
    Vu_waarde=VU_tot;
  //  dbgprint("vuwaarde totaal: %i",Vu_waarde);
  //  Vu_waarde=(spectrum[0][0] + spectrum[3][0] +spectrum[6][0] + spectrum[10][0])/4;
   // Vu_waarde=spectrum[6][0];
}
...
void readIOprefs()
{
  ...
  struct iosetting klist[] = {                            // List of I/O related keys
    ...
    { "pin_ch376_cs",  &ini_block.ch376_cs_pin,     34 }, // CH376 CS for USB interface
    { "pin_ch376_int", &ini_block.ch376_int_pin,    35 }, // CH376 INT for USB interfce
...
String getradiostatus()
{
 ...
 static char         tmpcmd[11];
 String           pnrmd;
 //value="precon_";
 sprintf ( tmpcmd, "precon_%02d", ini_block.newpreset ) ;         // Current preset
 dbgprint("mark iniblock newpreset waarde=%s",tmpcmd);

nvsopen();
if(nvsgetstr(tmpcmd)=="")
{
  dbgprint("No favicon in ini file for this station");
  dbgprint("Setting Default image instead");
  pnrmd= "http://www.judoles.nl/Projecten/Logo200.png";
}
else
{
  pnrmd=nvsgetstr(tmpcmd); 
}
...
void getsettings()
{
  ...
  val += getradiostatus() +                              // Add radio setting
         String ( "\n\n" ) ;                             // End of reply
  dbgprint("radiostatus\n%s",getradiostatus().c_str());       
...
void setup()
{
  ...
  strip.begin();
  striplogo.begin(); 
  strip.show(); // nothing to show yet...so this is a making sure all leds are off
  striplogo.show(); // nothing to show yet...so this is a making sure all leds are off
  strip.setBrightness(PIXBRIGHTNESS);
  striplogo.setBrightness(PIXBRIGHTNESSLOGO);
  
  for (int ledtmp=0;ledtmp< N_PIXELS_LOGO;ledtmp++){
  striplogo.setPixelColor(ledtmp,150,0,0);
  striplogo.show();
 ...
 // if ( find_html_version    < 172222 ) dbgprint ( wvn, "find" ) ;
...
//************************
       ...
       ( ini_block.tft_scl_pin >= 0 ) )
  {
    ...
    if ( dsp_begin() )                                   // Init display
    {
      ...
      dsp_setCursor ( 0, 0 ) ;
      tft->drawRGBBitmap((tft->width()-INTRO_WIDTH)/2,tft->height()-INTRO_HEIGHT, IntroBitmap, INTRO_WIDTH, INTRO_HEIGHT);
    
     //  tft->drawRGBBitmap(DRAGON_WIDTH,DRAGON_HEIGHT, dragonBitmap, DRAGON_WIDTH, DRAGON_HEIGHT);
     // tft->drawRGBBitmap(random(-DRAGON_WIDTH , tft->width()), random(-DRAGON_HEIGHT, tft->height()), dragonBitmap, DRAGON_WIDTH, DRAGON_HEIGHT);
      ...
      dsp_println ( tmpstr ) ;
      dsp_println ( "By Ed Smallenburg, Lotfi and The electronic Engineer" ) ;
    ...
    if ( cmdclient.connected() )
    {
      ...
      else
      {
        if ( http_getcmd.length() )                         // Command to analyze?
        {
          ...
          { dbgprint("yes mp3 list commando ontvangen");
            if ( datamode != STOPPED )                      // Still playing?
            {
              setdatamode ( STOPREQD ) ;                    // Stop playing
            }
            cmdclient.print ( sndstr ) ;                    // Yes, send header
            dbgprint("header retour text: %s",sndstr.c_str());
...
void handleFSf ( const String& pagename )
{
  ...
  else
  {
    ...
    else if ( pagename.indexOf ( "Intro.png" ) >= 0 ) // Favicon icon is in PROGMEM
    {
      p = (char*)IntroBitmap ;
      l = sizeof ( IntroBitmap ) ;
    }
 
    else if ( pagename.indexOf ( "find.html" ) >= 0 ) // find pafe is in PROGMEM
    {
      p = find_html ;
      l = sizeof ( find_html ) ;
    } 
...
const char* analyzeCmd ( const char* par, const char* val )
{
  ...
  String             value2;                         //mark
  String             piclink;                         //mark
  String             statname;                         //mark
  String             staturl;                         //mark
  String             statnum;                         //mark
   String             statnum2;                         //mark 
  ...
  char                tmp;                            //mark
  String             urltemp[200];                         // added by mark
  String             StationPresetnbr;                 // added by mark
  String             StationNme[100];                       // added by mark
  String             Setstatimg;                      // added by mark
  String             Setstat;                      // added by mark
  int                inxtmp;                     // added by mark
  int                inxtmp2;                     // added by mark
  int                cnttmp;                    // added by mark
  ...
  else if ( argument.startsWith ( "hostplaying" ) )        // Update request
  {
     // When someone opens a new browser to connect to the webserver, radio data and icon are not visible.
     // Because the webserver needs to fetch data to display first
   //  sprintf ( reply, "now playing:=%s", host.c_str()) ; // this gives the actual URL playing  
   // value2= String (currentpreset);
     sprintf ( reply, "precon_0%s", String(currentpreset)) ;
    
    nvsopen();
   // value="precon_0"+tmpstr;
    value2=nvsgetstr ( reply );
         sprintf ( reply, "%s",value2.c_str());
    
    // dbgprint("preset: %s",currentpreset);
     //currentpreset
     /*
     nvsopen(); // make sure the NVS is accessible
     for (cnttmp=0;cnttmp<8;cnttmp++)
     {
      sprintf(reply,"preset_0%d",cnttmp);
      value2= nvsgetstr (reply);
      dbgprint("%s",value2.c_str());
      // now lets find out if it contains our host
      dbgprint("host=%s",host.c_str());
      value= value2.indexOf(host.c_str());
      dbgprint("gevonden:%s",value);
     }
     */
  }
  
  else if ( argument.startsWith ( "setmem" ) )        // Update request
  { 

    value2=value; 
    value2.remove ( 1, value2.length() ) ;// bevat nu station nummer
    statnum="preset_0"+value2;//iets mee doen
    statnum2=value2;
    value2=value;  //1__stationname_url_=stationURL_icon_=favicon
    value2.remove(0,3); // stationname_url_=stationURL_icon_=favicon
    value2.remove(value2.indexOf("_url_="),value2.length()-value2.indexOf("_url_=")); //stationname
    statname=value2;
  
    value2=value;    //1__stationname_url_=stationURL_icon_=favicon
    value2.remove(value2.indexOf("_icon_="),value2.length()-value2.indexOf("_icon_=") );//1__stationname_url_=stationURL
            
    value2.remove(0, value2.indexOf("_url_=")+6); //stationURL
    inxtmp=value2.indexOf ( "http://" ) ;         // Search for "http://"
    inxtmp2=value2.indexOf ( "https://" ) ;         // Search for "https://"
    if ( inxtmp >= 0 )                                // Does URL contain "http://"?
    {
     staturl = value2.substring ( inxtmp + 7 ) ;      // Yes, remove it and set host
    }
    else if (inxtmp2 >=0)
    {
     staturl = value2.substring ( inxtmp2 + 8 ) ; // Yes, remove it and set host
    }
    else
    {
    staturl=value2;   // none of the above, no need to remove anything
    }

    
    
    value2=value;//1__stationname_url_=stationURL_icon_=favicon
    value2.remove(0,value2.indexOf("_icon_=")+7);//favicon
    piclink=value2;
     sprintf ( reply, "O yeah command setmemt=%s", value2.c_str()) ;

     statname.replace("%20"," "); // remove URL %20space characters and replace them with regular space
     statname.replace("%27"," ");
    dbgprint("stationnumber: %s",statnum.c_str());
    dbgprint("stationname: %s",statname.c_str());
    dbgprint("station URL: %s",staturl.c_str());
    dbgprint("favicon url: %s",piclink.c_str());
 
    // now time to write it to the NVS partition
   // nvsopen();
    //nvssetstr ( const char* key, String val )
   // setstatimg=
    //sprintf(setstat, 
    Setstat=staturl + " # " + statnum2 + " - "+ statname ;
    Setstatimg= piclink + "";
    dbgprint ("toSafe:%s",Setstat.c_str());
    nvsopen();
    nvssetstr(statnum.c_str(),Setstat);
    // nu nog afbeelding opslaa
    statnum.replace("preset_","precon_");
    // nvsopen();
     nvssetstr(statnum.c_str(),Setstatimg);
dbgprint("statnum voor picto:%s",Setstatimg);
    // scherm instellingen verversen in html
  }
 
  else if (argument == "iconid")
  { nvsopen();
    value="precon_0"+tmpstr;
    value2=nvsgetstr ( value.c_str() );
         sprintf ( reply, "%s",value2.c_str());  
     // sprintf ( reply, "%s",value2.c_str());   
  }
  
  else if ( argument == "test" )                      // Test command
  { //dbgprint("test pressed");
    if ( localfile )
    {
...
void displayinfo ( uint16_t inx )
{
  ...
  if ( tft )                                               // TFT active?
  {
    dsp_fillRect ( 0, p->y, width, p->height, BLACK ) ;    // Clear the space for new info //+10 is extra
    if ( ( dsp_getheight() > 64 ) && ( p->y > 1 ) )        // Need and space for divider?
    {
      dsp_fillRect ( 0, p->y - 4, width, 1, GREEN ) ;      // Yes, show divider above text//4
    }
    ...
    if ( len++ )                                           // Check string length, set buffer length
    {
      ...
      dsp_setCursor ( 0, p->y ) ;                          // Prepare to show the info +4 is extra
    ...
    for (uint8_t i = 0; i < bands; i++) // Handle all sections
    {
        if (visual) {
            if (spectrum[i][0] > spectrum[i][1]) {
                dsp_fillRect (posi, Spectrum_y0 + Spectrum_hy - spectrum[i][0], larg, spectrum[i][0], YELLOW);
        ...
        if (visual)
            dsp_fillRect (posi, Spectrum_y0 + Spectrum_hy - spectrum[i][2] - 3, larg, 2, CYAN);
...
void handle_spec()
{
  ...
  int n, height;// first neopix stuff
   int bright=0; 
  ...
  if (millis() >= millisbands + 50) {
    vs1053player->getBands() ;
    displaySpectrum();
    updateLogo();
    millisbands = millis();

                                   // now read the adc and adjust the brightness of Uv meter accordingly
    bright = analogRead(Brightpin);
    bright=map(bright, 0, 4095, 0, 128 );
...
void displaysignal()
{
    uint8_t posx = dsp_getwidth() + TIMEPOS - 20;//90
    uint8_t posy = 1;
    if ( NetworkFound && tft)
    {
        uint8_t mini = 90;
...
{ 
  while ( true )
  {
    ...
    vTaskDelay ( 50 / portTICK_PERIOD_MS ) ;                       // Pause for a short time 100
...
void VuUpdate() {
 ...
 //dbgprint("dempende waarde; %i",lvl);
  // Calculate bar height based on dynamic min/max levels (fixed point):
  height = TOP * (lvl - minLvlAvg) / (long)(maxLvlAvg - minLvlAvg);
 // height = (TOP * (lvl - minLvlAvg) / (long)(maxLvlAvg - minLvlAvg))-50;
// dbgprint("height: %i",height);
...
// Every few frames, make the peak pixel drop by 1:
  ...
  maxLvlAvg = (maxLvlAvg * 63 + maxLvl+50) >> 6; // (fake rolling average) +50 at end is to prevent offscale all the time
// maxLvlAvg=maxLvlAvg+50;
// dbgprint("minLvlAvg: %i", minLvlAvg);
// dbgprint("maxLvlAvg: %i", maxLvlAvg);
}
...
void updateLogo(){
 ...
 for(int i=0; i<striplogo.numPixels(); i++) { // For each pixel in strip...
      int pixelHue = firstPixelHue + (i * 65536L / striplogo.numPixels());
      striplogo.setPixelColor(i, striplogo.gamma32(striplogo.ColorHSV(pixelHue)));
    }
   firstPixelHue += 256;
    striplogo.show(); // Update strip with new contents
